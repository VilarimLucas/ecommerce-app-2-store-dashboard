...
const ProductView = () => {
  ...
  const [totalValue, setTotalValue] = useState(0); // Para armazenar o total dos produtos
  const [productCount, setProductCount] = useState(0); // Para armazenar a quantidade de produtos
  ...
  const fetchProducts = async () => {
    try {
      ...
      const total = productsData.reduce((sum, product) => sum + product.productPrice, 0);
      setTotalValue(total);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      fetchProducts(); // Recarrega a lista após exclusão
    } catch (err) {
